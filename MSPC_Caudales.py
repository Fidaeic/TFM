...
class PCA(object):
    def __init__(self, autoescalado = True, tolerancia = 1e-15, verbose = False):
    ...
    def explain_plot(self):
        ...
        source = ColumnDataSource(data=dict(
            x  = range(1,25),
            value =list(self.rsquared_acc*100),
            ))
        ...
        p.vbar(x = data.index, top = data['rsquared'], width=0.8)
        show(p)
    ...
    def score_plot(self, comp1, comp2):
        ...
        p1.circle(data[comp1], data[comp2], size=5, color="navy", alpha=0.5)
    ...
    def loadings_plot(self, comp):
        ...
        p.vbar(x = range(P_t.shape[1]), top = data[comp], width=0.8)
